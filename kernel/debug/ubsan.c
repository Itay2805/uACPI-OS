...
void __ubsan_handle_negate_overflow(overflow_data_t* data, size_t ozd_val) {
    bool is_signed = is_signed_integer_ty(data->type);

    if (is_signed) {
        LOG_UBSAN("negation of %zd cannot be represented in type %s; "
            "cast to an unsigned type to negate this value to itself",
            ozd_val, data->type->type_name);
    } else {
...
void __ubsan_handle_pointer_overflow(pointer_overflow_data_t* data, size_t base, size_t result) {
    if (base == 0 && result == 0) {
        ...
        LOG_UBSAN("applying non-zero offset %zd to null pointer", result);
    } else if (base != 0 && result == 0) {
        LOG_UBSAN("applying non-zero offset to non-null pointer %zx produced null pointer", base);
    } else if (((intptr_t)base >= 0) == ((intptr_t)result >= 0)) {
        if (base > result) {
            LOG_UBSAN("addition of unsigned offset to %zx overflowed to %zx", base, result);
        } else {
            LOG_UBSAN("subtraction of unsigned offset to %zx overflowed to %zx", base, result);
        }
    } else {
        LOG_UBSAN("pointer index expression with base %zx overflowed to %zx", base, result);
    }
}
...
void __ubsan_handle_function_type_mismatch(function_type_mismatch_data_t* data, size_t function) {
    LOG_UBSAN("call to function %zx through pointer to incorrect function type %s", function, data->type->type_name);
}
...
void __ubsan_handle_type_mismatch_v1(type_mismatch_data_t* data, size_t pointer) {
    ...
    if (pointer == 0) {
        ...
        LOG_UBSAN("%s misaligned address %zx for type %s, "
            "which requires %zd byte alignment",
            m_type_check_kinds[data->type_check_kind], pointer, data->type->type_name, alignment);
    } else {
        LOG_UBSAN("%s address %zx with insufficient space "
