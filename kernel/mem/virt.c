...
err_t virt_map_page(uint64_t phys, uintptr_t virt, map_flags_t flags) {
    ...
    CHECK((phys % PAGE_SIZE) == 0);
    CHECK((virt % PAGE_SIZE) == (phys % PAGE_SIZE));

    page_entry_t* pml3 = get_next_level(&m_cr3[PML4_INDEX(virt)]);
    CHECK_ERROR(pml3 != NULL, UACPI_STATUS_OUT_OF_MEMORY);

    page_entry_t* pml2 = get_next_level(&pml3[PML3_INDEX(virt)]);
    CHECK_ERROR(pml2 != NULL, UACPI_STATUS_OUT_OF_MEMORY);

    page_entry_t* pml1 = get_next_level(&pml2[PML2_INDEX(virt)]);
    CHECK_ERROR(pml1 != NULL, UACPI_STATUS_OUT_OF_MEMORY);
...
err_t virt_alloc_range(uintptr_t virt, size_t page_count) {
    ...
    for (i = 0; i < page_count; i++) {
        ...
        void* phys = phys_alloc(PAGE_SIZE);
        CHECK_ERROR(phys != NULL, UACPI_STATUS_OUT_OF_MEMORY);
...
err_t init_virt() {
    ...
    for (int i = 0; i < ehdr->e_phnum; i++) {
        ...
        TRACE("memory: %lx-%lx (%lx-%lx) [%c%c%c] %08lx",
                 vaddr, vend,
                 paddr, pend,
...
bool virt_handle_page_fault(uintptr_t addr) {
    ...
    if (0xFFFFC00000000000 <= addr && addr < 0xFFFFD00000000000) {
        // kernel heap
